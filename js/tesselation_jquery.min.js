...
//////////////////////////////
// helper for DOM visibility at page finished loading
// this comes from here : https://www.primative.net/blog/how-to-get-rid-of-the-flash-of-unstyled-content/
//////////////////////////////
...
function adjust_tesselation_viewbox(){
  var width = (window.innerWidth > 0) ? window.innerWidth : screen.width;
  var height = (window.innerHeight > 0) ? window.innerHeight : screen.height;
  
  
  var str_width =  Math.floor(width).toString();
  var str_height = 0;
  
  
  
  if (width > 800){
	str_height =  Math.floor((height*0.4) - ((height*0.4)%5)).toString();
  }
  else {
	str_height = Math.floor((height*0.7) - ((height*0.7)%5)).toString();
  }
  document.getElementById('rect1').setAttribute("width",str_width);
  document.getElementById('rect1').setAttribute("height",str_height);
  document.getElementById('rect2').setAttribute("width",str_width);
  document.getElementById('rect2').setAttribute("height",str_height);
  document.getElementById('animated_background').setAttribute("viewBox", "0 0 " + str_width + " " + str_height);
}
...
function init(showStats) {
  ...
  // init
  adjust_tesselation_viewbox();
  var svg = document.getElementById('animated_background');
  //alert(svg);
  tesselation.setup(svg);
  gradients.setup();

  var lastTransitionAt, transitionDelay = 20000, transitionDuration = 5000;
  ...
  function wait_end_of_resize(){
	  clearTimeout(resize_timeout);
	  resize_timeout = setTimeout(resize_handler, 1000);
  }
  ...
  window.requestAnimationFrame(tick);
  window.onfocus = recover_focus;
  window.addEventListener('resize',wait_end_of_resize)
}
...
var tesselation = (function() {
  ...
  function createRandomTesselation() {
	...
	var wW = document.getElementById('rect1').getAttribute("width")
	var wH = document.getElementById('rect1').getAttribute("height")
	
	var gridSpacing = wH/5, scatterAmount = 0.5;
	var gridSize, i, x, y;
	...
	for (x = Math.floor(svgW/gridSize) +1 ; x >= -1; x--) {
	  for (y = Math.floor(svgH/gridSize) ; y >= 0; y--) {
		if (y > 0 && y < Math.floor(svgH/gridSize)){
		  vertices.push(
			[
...
var gradients = (function() {
  ...
  return {
	...
	next: function(t) {
	  ...
	  TweenMax.to(hide.rect, 0.55*t, {opacity: 0, delay: 0.5*t, ease: Sine.easeOut});
